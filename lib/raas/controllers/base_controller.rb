  ...
  class BaseController
    ...
    def validate_response(context)
      if context.response.status_code == 400
        raise RaasClientException.new('Bad Request',
                                      context)
      elsif context.response.status_code == 401
        raise RaasGenericException.new('Unauthorized - Invalid Credentials',
                                       context)
      elsif context.response.status_code == 403
        raise RaasClientException.new('Forbidden',
                                      context)
      elsif context.response.status_code == 404
        raise RaasGenericException.new('Not Found',
                                       context)
      elsif context.response.status_code == 409
        raise RaasClientException.new('Conflict',
                                      context)
      elsif context.response.status_code == 500
        raise RaasServerException.new('Internal Server Error - Retry Later',
                                      context)
      elsif context.response.status_code == 503
        raise RaasServerException.new('Service Unavailable - Retry Later',
                                      context)
      end
